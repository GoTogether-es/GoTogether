...
### Medium
- **Explorar** usa la ubicación guardada del usuario para priorizar acompañantes cercanos
...
- **console.warn**: solo en development (5 sitios)
...
- **Admin controller**: `UpdateBookingStatusDto` con `@IsEnum(BookingStatus)` en vez de `as any`
...
- Eliminada función muerta `isPublicRoute` que referenciaba constante borrada
...
- **Login**: validación client-side de email (regex + error inline), mensajes específicos (red vs API vs genérico), hint de carpeta spam
- **Solicitud**: `SkeletonForm` en vez de pantalla en blanco durante carga, Cancelar vuelve a `/explorar`
- **Perfil**: JSX reestructurado (LocationSharingCard/PanelLink fuera del card de compañero)
...
- **Registro compañero**: preview en vivo de tags de especialidades
...
- **Supervisión**: skeletons de carga en pestaña de reservas
- **Admin**: spinners de carga en tabs + `window.confirm()` al rechazar + validación client-side en formulario de servicios
- **Valoración**: navegación de estrellas con teclado (flechas + tabIndex)
- **Registros**: botones "Volver" a selección de rol

### Build
- **Webpack alias** para `zod/v4/core` en Next.js (compatibilidad ESM/CJS)
- Build corregido: 3 bugs de JSX desbalanceado, imports perdidos, tipos faltantes
